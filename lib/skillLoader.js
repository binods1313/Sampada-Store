...
class SkillLoader {
  ...
  async loadSkills() {
    try {
      ...
      if (shouldLog) console.log(`Found ${skillDirs.length} skills: ${skillDirs.join(', ')}`);
      ...
      if (shouldLog) console.log(`Successfully loaded ${this.skills.size} skills`);
    } catch (error) {
  ...
  async loadSkill(skillName) {
    ...
    const shouldLog = process.env.NODE_ENV === 'production' || process.env.DEBUG_KIRO === 'true';
    if (shouldLog) console.log(`✓ Loaded skill: ${skillName}`);
  }
