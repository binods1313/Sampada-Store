...
class RuntimeRegistry {
  ...
  async initialize() {
    if (this.initialized) {
      console.log('Runtime registry already initialized');
      return;
    }

    console.log('Initializing runtime registry...');
    ...
    try {
      ...
      console.log(`✓ Runtime registry initialized with ${this.registry.powers.size} powers and ${this.registry.skills.size} skills`);
  ...
  get(name) {
    ...
    if (this.registry.skills.has(name)) {
      ...
      if (this.registry.conflicts.some(c => c.name === name)) {
