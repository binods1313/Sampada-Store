...
class PowerLoader {
  ...
  async loadPowers() {
    try {
      ...
      if (shouldLog) console.log(`Found ${powerDirs.length} powers: ${powerDirs.join(', ')}`);
      ...
      if (shouldLog) console.log(`Successfully loaded ${this.powers.size} powers`);
    } catch (error) {
  ...
  async loadPower(powerName) {
    ...
    const shouldLog = process.env.NODE_ENV === 'production' || process.env.DEBUG_KIRO === 'true';
    if (shouldLog) console.log(`✓ Loaded power: ${powerName}`);
  }
