...
export class ErrorHandler {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('unhandledrejection', (event) => {
        if (this.isExtensionError(event.reason)) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.debug('[ErrorHandler] Ignored extension rejection:', event.reason?.message || event.reason);
          }
          return;
        }
        this.handleError(
      ...
      window.addEventListener('error', (event) => {
        ...
        if (this.isExtensionError(error)) {
          event.preventDefault();
          if (process.env.NODE_ENV === 'development') {
            console.debug('[ErrorHandler] Ignored extension error:', event.message || event.error?.message);
          }
          return;
        }
        this.handleError(
  ...
  handleError(error, showToast = true) {
    ...
    // FILTER: Ignore errors from browser extensions (e.g. MetaMask, Grammarly, etc.)
    // These are often "Failed to connect to MetaMask" or "chrome-extension://" errors
    // that the app didn't trigger and can't fix.
    const errorMessage = enhancedError.message || '';
    const errorStack = enhancedError.stack || '';
    ...
    if (isExtensionError) {
      // Log to console only, don't show toast or report to monitoring
      if (process.env.NODE_ENV === 'development') {
        console.debug('[ErrorHandler] Ignored extension error:', enhancedError.message);
      }
