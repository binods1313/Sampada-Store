...
import { writeClient, urlFor } from '../lib/client';
...
const AccountPage = ({ user, orders: initialOrders, error: serverError }) => {
  ...
  const refreshOrders = async () => {
    ...
    try {
      ...
      // Only update if we have orders or this isn't our first refresh attempt
      if (data.orders?.length > 0 || refreshAttempts > 0) {
  ...
  // Loading state
  if (status === 'loading') {
    return <main style={pageStyles.mainContainer}><p>Loading Your Account...</p></main>;
  }
  ...
  return (
    <>
      <Head>
        <title>My Account | Binod Tech Ventures</title>
        <meta name="description" content="View account information and order history" />
      </Head>
      ...
      <main style={pageStyles.mainContainer}>
        ...
        {serverError && <p style={pageStyles.errorText}>Error loading data: {serverError}</p>}
             ...
             {orders && orders.length > 0 ? (
               ...
               !serverError && !isRefreshing && router.query.from !== 'success' && (
                 <div style={{ textAlign: 'center', padding: '30px 0' }}>
                   ...
                   <Link href="/" passHref>
                     <Button style={{backgroundColor: '#ef4444', color: 'white'}}> Start Shopping </Button>
                   </Link>
                 </div>
               )
             )}
...
export async function getServerSideProps(context) {
  ...
  try {
    const sanityUser = await writeClient.fetch(`*[_type == "user" && providerId == $authUserId][0]`, { authUserId: userId });
